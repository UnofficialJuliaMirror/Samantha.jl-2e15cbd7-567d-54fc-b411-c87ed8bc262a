...
const EDGE_PATTERNS = Dict{Type,Vector{Tuple{Dict,Function}}}()

# Stores an edge pattern for a specific node type
macro edgepattern(nodetype, pattern::Expr, sizefunc::Expr)
  quote
    if !haskey(EDGE_PATTERNS, $nodetype)
      EDGE_PATTERNS[$nodetype] = Tuple{Dict,Function}[]
    end
    push!(EDGE_PATTERNS[$nodetype], (Dict($pattern), $sizefunc))
  end
end
